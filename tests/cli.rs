...
/// Selecting by content id is a batch operation: read the ids from `list --json`, stage them
/// together. Resolving each id by a linear scan over every sub-hunk's hash made that quadratic
/// in the size of the diff — the same shape as the auto-split regression above, one level up.
/// The ids are looked up in an index built once per invocation, so this stays linear.
#[test]
fn many_ids_resolve_without_quadratic_blowup() {
    ...
    let listing = String::from_utf8(common::run_ok(&["list", "--json"], &diff)).unwrap();
    // Every `"id": "…"` in the listing, in order — one per sub-hunk.
    let ids: Vec<String> = listing
        ...
        .map(|id| format!("@{id}"))
        .collect();
    assert_eq!(ids.len(), RUNS, "one id per change run");

    let args: Vec<&str> = std::iter::once("select")
        .chain(ids.iter().map(String::as_str))
        .collect();
    let started = std::time::Instant::now();
    let out = common::run_ok(&args, &diff);
    let elapsed = started.elapsed();

    assert_eq!(
        String::from_utf8_lossy(&out).matches("@@ -").count(),
        RUNS,
        "every named sub-hunk is emitted"
    );
    // Same reasoning as the auto-split guard: linear resolution of this input is a fraction of a
    // second even in a debug build, so a minute cannot be reached without a change in complexity.
    assert!(
        elapsed < std::time::Duration::from_secs(60),
